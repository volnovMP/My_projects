...
class PACKAGE TCommPort : public TComponent
{
	private:   //---------------------------------------------- частные (внутренние) объекты
		...
		DWORD __fastcall GetOutBuffFree(void);//узнать размер свободного места в буфере вывода
...
//------------------------------------------------------------ Com Config ================
//---------------------------------------------------------------------- Потоки компонента
		TReadThread *ReadThread; //-------------------------- поток, ожидающий приема символов
		TWriteThread *WriteThread; //------------------------ поток, ожидающий записи символов
		TStatusThread *StatusThread; //-------------- поток, следящий за всеми событиями порта
		TReadEventThread *ReadEventThread; //----- поток, ожидающий завершения чтения из порта
		TStatusEventThread *StatusEventThread;//поток,сообщающий из компонента о статусе линий
