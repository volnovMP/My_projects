...
#include "opred.h"
  ...
  unsigned char VVOD_DC[45*Nst][11];
  int noviznaDC[30*Nst],peredalDC[30*Nst],priniatoDC[30*Nst];
 	int ukaz_DC[Nst];
	unsigned char VVOD_NEPAR[Nst][70][7];
	unsigned char BUF_OUT_ARM[70],OUT_TO_ARM[70];
	char BUF_OUT_DC[70];
	unsigned char BUF_IN_ARM[28]; //------------------- буфер приема данных из программы ДСП
	unsigned char BUF_OUT_SOSED[28]; //------- буфер вывода данных в соседнюю ПЭВМ (АРМ ДСП)
	unsigned char BUF_IN_SOSED[28]; //------- буфер приема данных из соседней ПЭВМ (АРМ ДСП)
  ...
  long *POOO;
	...
	tiki_tum[Nst];
  bool Upr_DC; //-------------------------------------- признак включения управления от ДС
  ...
  OK_CIAN, //------------------------------------ текущий признак непарафазности кнопки Ок
  OK_CIAN_OLD, //-------------------------------- прежний признак непарафазности кнопки Ок
  StatCIAN,//--------------------- признак непарафазности переключателя основной/резервный
  Old_StatCIAN,
	KNOPKA_OK_N, //-------------------------------------- признак состояния 2-ух каналов КОК
	KNOPKA_OK_ARM, //-------------------------------------- признак нажатия кнопки ОК на АРМ
...
//========================================================================================
  ...
  Timer_KOK_DC, //---- таймер КОК для работы в ДЦ (фиксация времени выдачи исполнительной)
  ...
  int Norma_DC; //----------------------------------------- признак приема квитанций от ДЦ
  bool Nach_otv ,//------------- признак выданной предварительной ответственной команды ДЦ
  Kon_otv;//--------------------- признак выданной исполнительной ответственной команды ДЦ
