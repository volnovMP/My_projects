...
begin
  ...
  Tablo1.Free;
  Tablo2.Free;
  if DirectFormDlg.Active then DirectFormDlg.Close;
//  MsgFormDlg.Free;
end;

procedure TTabloMain.FormCreate(Sender: TObject);
  var err: boolean; i : integer; cok : string;
begin
  ...
  begin
    if reg.ValueExists('arcpath') then config.arcpath := reg.ReadString('arcpath') else config.arcpath := 'c:\arc';
    if reg.ValueExists('MaxOld') then
    begin // загрузить продолжительность доступа к старым файлам архива
      i := reg.ReadInteger('MaxOld'); ArcMaxOldTime := i;
    end else ArcMaxOldTime := 31.0;
    reg.CloseKey;
  end;
  if Reg.OpenKey(KeyNameDsp, false) then
  begin
    if reg.ValueExists('databasepath') then database := reg.ReadString('databasepath') else err := true;
    if reg.ValueExists('path') then config.path := reg.ReadString('path') else err := true;
    if reg.ValueExists('ru') then config.ru := reg.ReadInteger('ru') else err := true;
    if reg.ValueExists('RMID') then config.RMID := reg.ReadInteger('RMID') else err := true;
    reg.CloseKey;
...
begin
  ...
  if configRU[config.ru].TabloSize.X > 0 then DirectFormDlg.Region.ItemIndex := config.ru-1;
...
begin
  ...
  try
    ...
    if NewNeisprav then
    begin
      MsgFormDlg.Memo.Text := ListNeisprav;
      MsgFormDlg.MemoUVK.Text := ListDiagnoz;
      if SndNewWar then begin playsound('media\sound1.wav',0,SND_ASYNC); SndNewWar := false; end else
      if SndNewUvk then begin playsound('media\sound2.wav',0,SND_ASYNC); SndNewUvk := false; end else
      if SndNewMsg then begin playsound('media\sound6.wav',0,SND_ASYNC); SndNewMsg := false; end;
      SndNewWar := false; SndNewMsg := false;
    end;
  finally
