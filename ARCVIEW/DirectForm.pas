...
var
  ...
  YStep  : Boolean; //---------------------------------- признак включения прокрутки клипа
...
begin
  ...
  begin YStep:= false;ShowMessage('Конец фрагмента '); result:= Length(arhiv); exit; end;
  ...
  begin
    case TypeLastFrame of
      ...
      begin
        ...
        end else CmdMsg(imsg,ifr,i);//---------------- подтверждение команды или выдача ТУ
      end;
