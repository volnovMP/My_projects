...
uses
{$IFDEF RMDSP}  TabloForm, commands, {$ENDIF}
...
begin
  try
    RcvOLS[1].hEvent := INVALID_HANDLE_VALUE;
    TrmOLS[1].hEvent := INVALID_HANDLE_VALUE;
...
try
  ...
  begin // труба
    ...
    begin
      ...
      case kanal of
        1 : th := CreateThread(nil,0,@ReadSoobPipe,nil,0,Dummy); // создание потока трубы1
        2 : th := CreateThread(nil,0,@ReadSoobPipe2,nil,0,Dummy);// создание потока трубы2
      end;
...
try
  ...
  reportf('Ошибка [KanalArmSrv.DisconnectKanalSrv]');
...
//----------------------------------------------- Процедура потока обработки каналов связи
...
begin
  try
    ...
    begin
      SyncReady;
      sleep(50);
      //WaitForSingleObject(hWaitKanal,INFINITE);
    end;
    ExitThread(0);
    result := 0;
  except
    reportf('Ошибка [KanalArmSrv.SyncReadyThread]');
...
//========================================================================================
//---------------------------------------------- Периодические чтение/запись данных канала
function SyncReady : Boolean;
var
  i,j : integer;
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin //------------------------ зафиксировать восстановление связи с сервером
              AddFixMessage(GetShortMsg(1,434,'1',0),5,0);
              InsArcNewMsg(0,434+$2000,0);
            end;
            FixedConnect[1] := true;
            if i >= 70 then
            begin
            ...
            for j := 0 to i-1 do
            begin
              KanalSrv[1].RcvBuf[KanalSrv[1].RcvPtr] := KanalSrv[1].port.Buffer[j];
              inc(KanalSrv[1].RcvPtr);
            end;
            if KanalSrv[1].RcvPtr > LnSoobSrv * 5 then
            begin
              inc(KanalSrv[1].cnterr); //----------------- увеличить счетчик ошибок канала
              reportf('Переполнение входного буфера канала 1 ('+
              IntToStr(KanalSrv[1].RcvPtr)+' байт) '+ DateTimeToStr(LastTime));
              KanalSrv[1].RcvPtr := 0;
            end else ExtractSoobSrv(1); //--------------- распаковка данных из 1-го канала
            if MyMarker[1] then
        ...
        begin
          ...
          begin
            ...
            begin //------------------------ зафиксировать восстановление связи с сервером
              AddFixMessage(GetShortMsg(1,434,'2',0),5,0); //- Восстановление связи с УВК
              InsArcNewMsg(0,434+$2000,0);
            end;
            FixedConnect[2] := true;
            if i >= 70 then
            ...
            for j := 0 to i-1 do
            begin
              KanalSrv[2].RcvBuf[KanalSrv[2].RcvPtr] := KanalSrv[2].port.Buffer[j];
              inc(KanalSrv[2].RcvPtr);
            end;
            if KanalSrv[2].RcvPtr > LnSoobSrv * 5 then
            begin
              inc(KanalSrv[2].cnterr); //------------------- увеличить счетчик ошибок канала
              reportf('Переполнение входного буфера канала 2 ('+
              IntToStr(KanalSrv[2].RcvPtr)+' байт) '+ DateTimeToStr(LastTime));
              KanalSrv[2].RcvPtr := 0;
            end
            else  ExtractSoobSrv(2); //------------------ распаковка данных из 2-го канала
            if MyMarker[2] then
            begin //----------------------- выставить признак синхронизации по 2-му каналу
              MySync[2] := true;
              MyMarker[2] := false;
              KanalSrv[2].issync := true;
              result := true;
            ...
            begin //------------------------------------- прочитать данные из буфера трубы
              ...
              begin
                ...
                begin
                  ...
                  IntToStr(KanalSrv[1].RcvPtr)+' байт) '+ DateTimeToStr(LastTime));
                  KanalSrv[1].RcvPtr := 0;
                end else ExtractSoobSrv(1); //--------------- распаковка данных из сервера
              end;
          ...
          begin //-- если не было завершения обслуживания канала - фиксировать обрыв связи
            ...
            begin
              ...
              begin
                ...
                begin
                  AddFixMessage(GetShortMsg(1,433,'1',0),4,4);
                  InsArcNewMsg(0,433+$2000,0);  //------------- Восстановление связи с УВК
                end;
                FixedDisconnect[1] := false;
                FixedConnect[1] := false;
              end;
            end else
            begin
              FixedDisconnect[1] := true;
              LastTime := Date+Time;
        ...
        begin
          case KanalSrv[2].State of
            ...
            begin //------------------------------------- прочитать данные из буфера трубы
              if not FixedConnect[2] then
              begin //---------------------- зафиксировать восстановление связи с сервером
                AddFixMessage(GetShortMsg(1,434,'2',0),5,0);
                InsArcNewMsg(0,434+$2000,0); //---------------- Восстановление связи с УВК
              end;
              FixedConnect[2] := true;
              ...
              begin
                if KanalSrv[2].rcvcnt > 70 then inc(KanalSrv[2].cnterr)
                else KanalSrv[2].cnterr := 0;
                KanalSrv[2].isrcv := true;
                MySync[2] := true;
                ...
                else ExtractSoobSrv(2); //------------------- распаковка данных из сервера
              end;
            ...
            PIPE_ERROR_STATE :  KanalSrv[2].active := false; //-- перерыв связи с сервером
          ...
          begin //-- если не было завершения обслуживания канала - фиксировать обрыв связи
            ...
            begin
              ...
              { DisconnectKanalSrv(2);
                ...
                begin
                  AddFixMessage(GetShortMsg(1,433,'2'),4,4);
                  InsArcNewMsg(0,433+$2000);
                end;
...
//========================================================================================
//---------------------------------------------- Процедура передачи в канал для СОМ-портов
...
//========================================================================================
//--------------------------------------------------- Процедура передачи в канал для трубы
...
var
  ...
  i,j,k : integer;
  a,b,bl,bh : byte;
  w{$IFDEF RMDSP},wyr,wmt,wdy,whr,wmn,wsc,wmsc{$ENDIF} : word;
  {$IFDEF RMDSP}uts : TDateTime;{$ENDIF}
  cErr : cardinal;
begin
  try
    ...
    begin
      ...
      MySync[kanal] := false;
      if not //----------------------------------------------------------------- ЕСЛИ НЕТ:
      (SyncCmd or (DoubleCnt > 0)//------------------- синхронизации или установки времени
      or WorkMode.MarhRdy //--------------------- или готового маршрута для выдачи в канал
      or (Cmdcnt > 0)  //---------- или команды раздельного управления, готовой к передаче
      or (DoubleCnt > 0) //---------------- или данных Double готовых к передаче на сервер
      or (LastCRC[kanal] > 0)) //-------------------- или указателя на последнюю квитанцию
      then  exit;
      ...
      //------------------------------------------------------ формирование кода состояния
      b := config.ru; //------------------------------------------------- район управления
      if WorkMode.PushOK then b := b + $20; //-------------------------------- признак КОК
      if not WorkMode.OKError then b := b + $40; //--------------- признак исправности КОК
      if WorkMode.PushRU then b := b + $80; //--------------------- признак основного АРМа

      j := 0;
      WrBuffer[j] := #$AA; inc(j);  //------------------------ идентификатор начала пакета
      WrBuffer[j] := char(a); inc(j); //----------------------- маркер + адрес отправителя
      WrBuffer[j] := char(b); inc(j); //------------------------------- код состояния АРМа

{$IFDEF RMDSP}
      if WorkMode.ServerSync then  //--------- если выдается команда синхронизации времени
      begin
        if SyncCmd then
        begin //--------------------------- выдать команду синхронизации времени на сервер
          DoubleCnt := 0; SyncCmd := false; SyncTime := false;
          uts := Date + Time;
          DecodeTime(uts,whr,wmn,wsc,wmsc); DecodeDate(uts,wyr,wmt,wdy);
          WrBuffer[j] := char(cmdfr3_autodatetime); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(cmdfr3_autodatetime);
          w := wSc and $ff; WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          w := wMn and $ff; WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          w := wHr and $ff; WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          w := wDy and $ff; WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          w := wMt and $ff; WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          w := wYr - 2000;  WrBuffer[j] := char(w); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(w);
          WrBuffer[j] := char(0); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(0);
          WrBuffer[j] := char(0); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(0);
        end else
        if (DoubleCnt > 0) and (ParamDouble.Cmd = cmdfr3_newdatetime) then //- новое время
        begin //--------------------------------------------------------- Упаковка времени
          DoubleCnt := 0;
          WrBuffer[j] := char(ParamDouble.Cmd); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Cmd);
          WrBuffer[j] := char(ParamDouble.Index[1]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[1]);
          WrBuffer[j] := char(ParamDouble.Index[2]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[2]);
          WrBuffer[j] := char(ParamDouble.Index[3]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[3]);
          WrBuffer[j] := char(ParamDouble.Index[4]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[4]);
          WrBuffer[j] := char(ParamDouble.Index[5]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[5]);
          WrBuffer[j] := char(ParamDouble.Index[6]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[6]);
          WrBuffer[j] := char(ParamDouble.Index[7]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[7]);
          WrBuffer[j] := char(ParamDouble.Index[8]); inc(j);
          NewCmd[kanal] := NewCmd[kanal] + char(ParamDouble.Index[8]);
        end;
      end;
{$ENDIF}
...
{$IFDEF RMDSP}
        ...
        begin //-------------------- Маршрутная команда от ДСП готова к передаче на сервер
          ...
          NewCmd[kanal] := NewCmd[kanal] + char(MarhTracert[1].MarhCmd[10]) +
          char(MarhTracert[1].MarhCmd[1])+ char(MarhTracert[1].MarhCmd[2]) +
          char(MarhTracert[1].MarhCmd[3]) + char(MarhTracert[1].MarhCmd[4]) +
          char(MarhTracert[1].MarhCmd[5]) + char(MarhTracert[1].MarhCmd[6]) +
          char(MarhTracert[1].MarhCmd[7]) + char(MarhTracert[1].MarhCmd[8]) +
          char(MarhTracert[1].MarhCmd[9]);
        end;
{$ENDIF}
...
//          trmkvit := trmkvit + ',' + IntToStr(w)+ ':'+ IntToStr(kanal);
  ...
  reportf('Ошибка [KanalArmSrv.WriteSoobPipe]');
  Application.Terminate;
end;
end;
...
//========================================================================================
//--------------------------------- Потоковая функция обслуживания трубы - чтение канала 1
...
begin
  try
    ...
    if stime = ArchName then inc(ArcIndex)
    else begin ArcIndex := 0; ArchName := stime; end;
    sz := config.arcpath+ '\'+ ArchName+ '.ard';

    hfile := CreateFile(PChar(sz), GENERIC_WRITE, 0, nil, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
    if hfile = INVALID_HANDLE_VALUE then
    begin
      reportf('Ошибка '+IntToStr(GetLastError)+' открытия файла архива');
      result := false;
      exit;
    end;
    try
      ...
      begin
        ...
        dt := LastTime * 86400;
        idt := Trunc(dt);
        cidt := idt div $1000000; bhh := byte(cidt);

        idt := idt - cidt * $1000000;
        cidt := idt div $10000; bhl := byte(cidt);

        idt := idt - cidt * $10000;
        cidt := idt div $100; blh := byte(cidt);
        ...
        case c of
          1 :
          begin //--- новизна состояния из канала и текущее состояние диалога с оператором
            if (ArcIndex = 0) and not StartRM then
            begin //------------------------- сохранить полный архив в начале нового файла
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := 255; inc(len);
              buffarc[len] := 11; inc(len); //-------------- признак архива 1-125 объектов

              buffarc[len] := bll; inc(len);  //------------------- 4 байта времени данных
              buffarc[len] := blh; inc(len);
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to 125 do
              begin
                buffarc[len] := FR3[i]; inc(len);
                buffarc[len] := FR4[i]; inc(len);
              end;
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 12; inc(len); //---------- признак архива 126-250 объектов
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 13; inc(len); //---------- признак архива 251-375 объектов

                buffarc[len] := bll; inc(len);
                buffarc[len] := blh; inc(len);
                buffarc[len] := bhl; inc(len);
                buffarc[len] := bhh; inc(len);
                for i := 251 to 375 do
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 14; inc(len); //---------- признак архива 376-500 объектов
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 15; inc(len); //---------- признак архива 501-625 объектов
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 16; inc(len); //---------- признак архива 626-750 объектов
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 17; inc(len); //---------- признак архива 751-875 объектов
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 18; inc(len); //--------- признак архива 876-1000 объектов

                buffarc[len] := bll; inc(len);
                buffarc[len] := blh; inc(len);
                buffarc[len] := bhl; inc(len);
                buffarc[len] := bhh; inc(len);
                for i := 876 to 1000 do
                begin
                  buffarc[len] := FR3[i]; inc(len);
                  buffarc[len] := FR4[i]; inc(len);
                end;
              ...
              begin
                ...
                buffarc[len] := $00; inc(len);
                buffarc[len] := 255; inc(len);
                buffarc[len] := 19; inc(len); //-------- признак архива 1001-1125 объектов

                buffarc[len] := bll; inc(len);
                buffarc[len] := blh; inc(len);
                buffarc[len] := bhl; inc(len);
                buffarc[len] := bhh; inc(len);
                for i := 1001 to 1125 do
                begin
                  buffarc[len] := FR3[i]; inc(len);
                  buffarc[len] := FR4[i]; inc(len);
                end;
            ...
            begin //--------------------------------------------------------- команды меню
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewMenuC) + 5; inc(len);
              ...
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewMenuC) do
              begin
                buffarc[len] := byte(NewMenuC[i]); inc(len);
              end;
            end;
            ...
            begin //-------------------------------------- новизна FR3 по основному каналу
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewFR[1]) + 5; inc(len);
              ...
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewFR[1]) do
              begin
                buffarc[len] := byte(NewFR[1,i]);
                inc(len);
              end;
            end;
            if NewFR[2] <> '' then
            begin //------------------------------------- новизна FR3 по резервному каналу
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewFR[2]) + 5; inc(len);
              ...
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewFR[2]) do
              begin
                buffarc[len] := byte(NewFR[2,i]);
                inc(len);
              end;
            end;
            ...
            begin //------------------------------- команда управления по основному каналу
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewCmd[1]) + 5; inc(len);
              ...
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewCmd[1]) do
              begin
                buffarc[len] := byte(NewCmd[1,i]);
                inc(len);
              end;
            end;
            ...
            begin //------------------------------ команда управления по резервному каналу
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewCmd[2]) + 5; inc(len);
              ...
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewCmd[2]) do
              begin
                buffarc[len] := byte(NewCmd[2,i]);
                inc(len);
              end;
            end;
            ...
            begin //--------------------------------- Фиксация сообщения по новизне из FR3
              buffarc[len] := $ff; inc(len);
              buffarc[len] := $00; inc(len);
              buffarc[len] := Length(NewMsg) + 5; inc(len);
              buffarc[len] := 6; inc(len); //--------------------------- признак сообщения

              buffarc[len] := bll; inc(len);
              buffarc[len] := blh; inc(len);
              buffarc[len] := bhl; inc(len);
              buffarc[len] := bhh; inc(len);
              for i := 1 to Length(NewMsg) do
              begin
                buffarc[len] := byte(NewMsg[i]);
                inc(len);
              end;
            end;
          end;
          ...
          begin //------------------------------- сохранить 10-ти минутный архив состояний
            ...
            buffarc[len] := 255; inc(len);
            buffarc[len] := 11; inc(len); //---------------- признак архива 1-125 объектов
            buffarc[len] := bll; inc(len); //--------------------------------------- время
            buffarc[len] := blh; inc(len); //--------------------------------------- время
            buffarc[len] := bhl; inc(len); //--------------------------------------- время
            buffarc[len] := bhh; inc(len); //--------------------------------------- время
            for i := 1 to 125 do
            begin
              buffarc[len] := FR3[i]; inc(len);
              buffarc[len] := FR4[i]; inc(len);
            ...
            begin
              ...
              for i := 126 to 250 do
              begin
                buffarc[len] := FR3[i]; inc(len); buffarc[len] := FR4[i]; inc(len);
              end;
            end;
        ...
        begin
          reportf('Ошибка '+IntToStr(GetLastError)+' записи в файл архива.');
          result := false;
        end;
      end else
      begin
        reportf('Ошибка '+ IntToStr(GetLastError)+ ' при перемещении указателя в конец файла архива.');
        result := false;
      end;
    finally
      if not CloseHandle(hfile) then
      begin
        reportf('Ошибка '+ IntToStr(GetLastError)+' закрытия файла архива.');
        result := false;
      end;
    ...
    reportf('Ошибка [KanalArmSrv.SaveArc]');
    Application.Terminate;
    result := false;
  end;
end;
...
begin
  try
    if LastCRC[kanal] < High(BackCRC) then  //---------- если буфер квитанций не полон, то
    begin
      inc(LastCRC[kanal]); BackCRC[LastCRC[kanal],kanal]  := crc; //--добавить эту в буфер
      result := true;
    end else
    begin
      for i := 1 to High(BackCRC)-1 do
      BackCRC[i,kanal] := BackCRC[i+1,kanal];//иначе сдвинуть все предыдущие с потерей 1ой
      BackCRC[LastCRC[kanal],kanal] := crc;
      result := false;
    end;
  except
    reportf('Ошибка [KanalArmSrv.AddCheck]');
    Application.Terminate;
    result := false;
  end;
end;
...
//========================================================================================
//----------------------- Взять из памяти очередную CRC из числа принятых для квитирования
...
begin
  try
    ...
    if LastCRC[kanal] > 0 then
    begin
      crc := BackCRC[1,kanal]; //---------------------------- взять первую CRC из принятых
      for i := 1 to LastCRC[kanal]-1 do
      begin
        BackCRC[i,kanal] := BackCRC[i+1,kanal]; //--------- передвинуть весь массив вперед
        BackCRC[i+1,kanal] := 0;
      end;
      dec(LastCRC[kanal]);
      result := true;
    ...
    reportf('Ошибка [KanalArmSrv.SSecdCheck]');
    Application.Terminate;
    result := false;
  end;
end;
...
loop:
    ...
    begin
      if KanalSrv[kanal].RcvBuf[i] = #$AA then break;//---------- найти байт начала пакета
      inc(i);
    end;
    ...
    if KanalSrv[kanal].RcvBuf[LnSoobSrv-1] = #$55 then // конец пакета на месте считаем KS
    begin
      w := byte(KanalSrv[kanal].RcvBuf[LnSoobSrv-3]) +
      byte(KanalSrv[kanal].RcvBuf[LnSoobSrv-2]) * 256;

      pcrc := crc16_t(w); //-------------------------------------------- к.с. из сообщения
      crc := CalculateCRC16(@KanalSrv[kanal].RcvBuf[1],LnSoobSrv-4); //------ подсчет к.с.
      ...
      begin  //--------------------- проверки достоверности выполнены - распаковать данные
        ...
        if not AddCheck(kanal,crc) then //------------- добавить CRC для будущей квитанции
        begin  //-------- и, если при этом была потеря квитанций из-за переполнения буфера
          inc(KanalSrv[kanal].cnterr); //--------------------- то увеличить счетчик ошибок
          inc(KanalSrv[kanal].chklost); //--- и увеличить счетчик неотправленных квитанций
        end;

        j := 1; //------------------------------------------------------- начало сообщения

        //- адреса-маркеры отправителя-приемника+признак получения квитанции от УВК в STAN
        b := byte(KanalSrv[kanal].RcvBuf[j]);

        if (b and $80) = $80 then KVITOK := true //--- есть квитанция на последнюю команду
        else KVITOK := false; //----------------------- нет квитанции на последнюю команду

        mm := b and $0f; //----------------------------------------------- выделить маркер
        if (config.RMID = mm) and //---------------------- если маркер соответствует и ...
        (LastTime - LastRcv < AnsverTimeOut) then
        MyMarker[kanal] := true //------------------- установка признака получения маркера
        else  MyMarker[kanal] := false; //--------------- запрет захвата канала дсп-сервер
        ...
        SrvState := b;
        b := b shr 4;
        LastSrv[kanal] := b; //------------ Получить номер запрашивающего сервера в канале
        inc(j);

        if config.RMID = mm then
        StateRU := byte(KanalSrv[kanal].RcvBuf[j]);//-- состояние сервера если свой маркер

        WorkMode.LockCmd := ((StateRU and $10) = 0);
        SrvState := SrvState or (StateRU and 7);

        if WorkMode.LockCmd then SrvState := SrvState or 8;
        ...
        inc(j);
        //--------------------------------- обновить байт актуального состояние связи STAN
        ...
        begin
          ...
          w := bh * 256 + bl; //------------------------------- двухбайтный адрес-параметр
          ...
          begin
            b := byte(pb);
            if w = WorkMode.DirectStateSoob then //----------------------- если это сервер
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if (w > 0) and (w < 4096) then //----------- однобайтное состояние датчика FR3
            begin
              if (w = config.avtodst) then  //------------------- если объект автодействия
              begin
                if (pb > '') then
                begin
                  FR3inp[w] := pb;
                  NAS := false;
                  CHAS := false;
                  if pb = 'A' then NAS := true;
                  if pb = 'C' then begin NAS := true; CHAS := true; end;
                  if pb = 'B' then CHAS := true;
                end else
                begin
                  FR3inp[w] := '@';
                  NAS := false;
              ...
              begin
                if NAS then
                begin
                  AddFixMessage(GetShortMsg(1,431,' нечетное',2),4,2);
                  InsArcNewMsg(0,431+$2000,2);
                  ShowShortMsg(431, LastX, LastY, ' нечетное')
                end else
                begin
                  AddFixMessage(GetShortMsg(1,430,' нечетное',7),4,2);
                  InsArcNewMsg(0,430+$2000,7);
                  ShowShortMsg(430, LastX, LastY, ' нечетное');
                end;
              end;

              if Old_CHAS <> CHAS then
              begin
                if CHAS then
                begin
                  AddFixMessage(GetShortMsg(1,431,' четное',2),4,2);
                  InsArcNewMsg(0,431+$2000,2);
                  ShowShortMsg(431, LastX, LastY, ' четное')
                end else
                begin
                  AddFixMessage(GetShortMsg(1,430,' четное',7),4,2);
                  InsArcNewMsg(0,430+$2000,7);
                  ShowShortMsg(430, LastX, LastY, ' четное');
                end;
              end;
              ...
              begin //----------------------- если не специализированные датчики - считать
                ...
                begin
                  ...
                  if FR6[w] <> 0 then FR6[w] := 0;
                  if (NewFr6 shr 8) = w then
                  begin
                    if OldFr6 = NewFr6 then OldFr6 := 0;
                    NewFr6 :=0;
                  end;
                end;
                FR3s[w] := LastTime;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if (w > 5120) and (w < 6144) then   //-------- 4-х байтные данные ненормы плат
            begin
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            begin
              ...
              KanalSrv[kanal].iserror := true;
              MsgStateRM := 'Искажение данных в канале'+IntToStr(kanal)+' w='+IntToStr(w);
              MsgStateClr := 1;
              //------------------------------- Увеличить счетчик ошибок формата сообщения
              {
              if (errch[kanal] > 30) and not LockTablo then
              begin
                //------------------------------------------- Попытаться остановить сервер
                SendCommandToSrv(WorkMode.DirectStateSoob,cmdfr3_logoff,0);
                LockTablo := true; KanalSrv[kanal].active := false;
                reportf('Число ошибок формата данных более 30, DUMP записан в файл '+
                config.arcpath+ '\kanal'+IntToStr(kanal));
                FixStatKanal(1);
                FixStatKanal(2);
                Beep;
                ShowWindow(Application.Handle,SW_SHOW);
                ShowMessage('Ошибок формата данных > 30. Протокол в "DSP.RPT".');
                Application.Terminate;
              end;
              }
              break;
        ...
        begin
          //------- сбросить признак ошибки данных если не зафиксирован неизвестный формат
...
//========================================================================================
//----------------------------------------------- обработка квитанции, принятой из сервера
...
{$IFDEF RMSHN} WorkMode.CmdReady := false;{$ENDIF}
    if Obj > Word(WorkMode.LimitFRI) then
    begin
      reportf('Получена квитанция на несуществующий объект с индексом '+ IntToStr(Obj));
      exit;
    end;

    case Kvit of //--------------------------------------- переключатель по типу квитанции
    0 :
    begin //--------------------------------------- Успешное исполнение раздельной команды
      WorkMode.CmdReady := false; //------------ разблокировать прием команды от оператора
    end;

    1 :
    begin //------------------------------------ сообщение о выполнении маршрутной команды
      WorkMode.CmdReady := false; //------------ разблокировать прием команды от оператора
      im := 0;//------------------------------------- поиск индекса маршрута по номеру FR3
      for i := 1 to WorkMode.LimitObjZav do
      if ObjZav[i].TypeObj = 5 then    //------------------------------- если это светофор
      begin
      ...
      begin
        MsgStateRM := GetShortMsg(2,1,ObjZav[im].Liter,0);
        MsgStateClr := 2;
      end;
      InsArcNewMsg(im,1+$400,7); //--------------------  Выполняется установка маршрута от
    end;

    2 :
    begin //------------- Сообщение об отказе от выполнения команды раздельного управления
      WorkMode.CmdReady := false; //------------ разблокировать прием команды от оператора
{$IFDEF RMDSP}
      if CmdBuff.LastObj = Obj then
      begin
        MsgStateRM := GetShortMsg(2,2,ObjZav[CmdBuff.LastObj].Liter,0);
        MsgStateClr := 1;
        InsArcNewMsg(0,2+$400,1);
      end;
{$ELSE}
...
{$ENDIF}
    ...
    3 :
    begin //--------------------------------------- отказ от выполнения маршрутной команды
      WorkMode.CmdReady := false; //------------ разблокировать прием команды от оператора
      im := 0;  //----------------------------------- поиск индекса маршрута по номеру FR3
      for i := 1 to WorkMode.LimitObjZav do
      if ObjZav[i].TypeObj = 5 then  //--------------------------------------- если сигнал
      begin
        if ObjZav[i].ObjConstI[3] > 0 then
        if Obj = (ObjZav[i].ObjConstI[3] div 8) then begin im := i; break; end;
    ...
    4 : begin // ---------------------- сообщение о выполнении команды передачи на маневры
      ...
      for i := 1 to WorkMode.LimitObjZav do
        ...
        if ObjZav[im].RU = config.ru then
        begin
          MsgStateRM := GetShortMsg(2,4,ObjZav[im].Liter,7);
          MsgStateClr := 2;
        end;
    ...
    6 : begin //----------------------------------------------- отказ включения управления
      ...
      InsArcNewMsg(0,6+$400,1);
    end;
    7 :
    begin //------------------------------------------ сброс маршрутной команды по неудаче
      ResetMarhrutSrv(Obj); //-------------- установить признак сброса маршрута на сервере
    end;
    8..61 : begin
      MsgStateRM := GetShortMsg(2,Kvit,' FR['+IntToStr(Obj)+ ']',1);
      MsgStateClr := 1;
    end;
  end;
except
