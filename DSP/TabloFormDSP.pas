...
begin
  ...
  begin
    ...
    if (KanalSrv[1].nPipe = '') and (KanalSrv[2].nPipe = '')  then KanalType := 0
     ...
     else begin err := true; reportf('Неверно определен тип канала связи с сервером'); end;

    if reg.ValueExists('configok') then ConfigPortOK := reg.ReadString('configok')
    else  begin  err := true;  reportf('Нет ключа "configok"');  end;
    {
      if reg.ValueExists('configok1') then ConfigPortOK1 := reg.ReadString('configok1')
      else
      begin
        err := true;
        reportf('Нет ключа "configok1"');
      end;
      if reg.ValueExists('configok2') then ConfigPortOK2 := reg.ReadString('configok2')
      else
      begin
        err := true;
        reportf('Нет ключа "configok2"');
      end;
    }
    if reg.ValueExists('AnsverTimeOut') then AnsverTimeOut := reg.ReadDateTime('AnsverTimeOut')
    ...
    if reg.ValueExists('TimeOutRdy') then MaxTimeOutRecave := reg.ReadDateTime('TimeOutRdy')
    else begin err := true; reportf('Нет ключа "TimeOutRdy"'); end;

    if reg.ValueExists('IntervalAutoMarsh')
    then IntervalAutoMarsh := reg.ReadInteger('IntervalAutoMarsh')
  ...
  if CreateKOK then //-------------------- Подключить Com-порт кнопки ответственных команд
  begin err := true;  reportf('Ошибка порта кнопки ОК.'); Beep; end;

  if InitKOK then //--------- инициировать параметры Com-порта кнопки ответственных команд
  begin  err := true; reportf('Ошибка инициализации порта кнопки ОК.'); Beep; end;
...
{$ENDIF}
  ...
  asTestMode := $55;
//--------------------------------------------------- если порт ОК построен на базе RS-232
  if (ConfigPortOK <> '') and Assigned(PortOK) then //----------- если указан порт для КОК
  begin
    if not PortOK.OpenPort then //------------------------------ если порт КОК не открылся
    begin
      reportf('Ошибка инициализации порта кнопки ОК.');
    end
    else //----------------------------- если порт КОК в норме, то выдать DTR на кнопку ОК
    begin PortOK.DTROnOff(true); end;
  end;

  //------------------------------------ если порт КОК построен на базе двух портов RS-485
  if (ConfigPortOK1 <> '') and Assigned(PortOK1) then //-------- если существует 1-ый порт
  begin
    if not PortOK1.OpenPort then  //------------------ если первый порт ОК открыт неудачно
    reportf('Ошибка инициализации первичного порта кнопки ОК.')//---- дать знать об ошибке
    else PortOK1.StrToComm('тест первичного порта КОК');//-- иначе отправить в порт строку
  end;

  if (ConfigPortOK2 <> '') and Assigned(PortOK2) then //-------- если существует 2-ой порт
  begin
    if not PortOK2.OpenPort then  //------------------ если второй порт ОК открыт неудачно
    reportf('Ошибка инициализации вторичного порта кнопки ОК.')//------ дать знать об этом
    else PortOK2.StrToComm('тест вторичного порта КОК');//-- иначе отправить строку в порт
  end;
  //--------------------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    begin
      ...
      KanalSrv[1].State := PIPE_EXIT; //------- Завершение приема из программного канала 1
      KanalSrv[2].State := PIPE_EXIT; //------- Завершение приема из программного канала 2

      if DspToDspEnabled and (DspToDspThread <> INVALID_HANDLE_VALUE) then
      begin
        DspToDspBreak := true;
        ResumeThread(DspToDspThread); //------------------------ возобновить поток ДСП-ДСП
      end;

      if DspToArcEnabled and (DspToArcThread <> INVALID_HANDLE_VALUE) then
        begin
        DspToArcBreak := true;
        ResumeThread(DspToArcThread);//----------------------- возобновить поток ДСП-архив
      end;
...
begin
  try
    ...
    reportf('Ошибка [TabloFormDSP.FormPaint]');
    Application.Terminate;
  end;
end;
//========================================================================================
...
begin
  ...
  Tablo.Canvas.UnLock;
    ...
    reportf('Ошибка [TabloFormDSP.DrawTablo] fix='+ IntToStr(fix)); Application.Terminate;
  end;
end;
//========================================================================================
  ...
  try
    ...
    begin
      ...
      begin
        ...
        SelectCommand;
        if cur_obj > 0
        then SetCursorPos(ObjUPrav[cur_obj].Box.Right-2, ObjUPrav[cur_obj].Box.Bottom-2);
        exit;
      end;
    end;
  except
    reportf('Ошибка [TabloFormDSP.TTabloMain.DspPopupHandler]'); Application.Terminate;
  end;
end;
//========================================================================================
...
try
  ...
  reportf('Ошибка [TabloFormDSP.TTabloMain.SetPlakat]'); Application.Terminate;
end;
end;
//========================================================================================
...
begin
  try
    ...
    reportf('Ошибка [TabloFormDSP.TTabloMain.GetPlakat]');
    Application.Terminate;
  end;
end;
//========================================================================================
...
try
  ...
  reportf('Ошибка [TabloFormDSP.TTabloMain.DrawPlakat]'); Application.Terminate;
end;
end;
...
try
  ...
  reportf('Ошибка [TabloFormDSP.ResetAllPlakat]'); Application.Terminate;
end;
end;
...
begin
  ...
  begin
    ...
    if DspMenu.WC then  begin  DspCommand.Active := true; SelectCommand; end
    else
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              ShowShortMsg(515,LastX,LastY,'объекта зависимостей в строке '+
              IntToStr(cntObjZav));
              Beep;
            end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                InsNewArmCmd($7ffc,0);
                ShowShortMsg(515,LastX,LastY,'объекта табло в строке '+
                IntToStr(cntObjView));
                Beep;
              end;
            end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                ShowShortMsg(515,LastX,LastY,'объекта управления в строке '
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                InsNewArmCmd($7ffc,0);
                PutShortMsg(515,LastX,LastY,'буфера отображения в строке '+
...
{$ELSE}
        KeyOtvKomState; //---------------------------------------- проверить состояние КОК
        if ((StateRU and $20) = $20) and
        WorkMode.PushOK then WorkMode.OtvKom := true  //--------------- Есть разрешение ОК
        else WorkMode.OtvKom := false;//-------------------------------- Нет разрешения ОК
{$ENDIF}
        ...
        begin //------------------------------- дать короткий звонок требуемой тональности
          if Ip1Beep then
          begin Ip1Beep := false; PlaySound(PAnsiChar(IpWav.Strings[0]),0,SND_ASYNC); end
          else //----------------------------------------------------------- 1 приближение
          if Ip2Beep then
          begin
            Ip2Beep := false; PlaySound(PAnsiChar(IpWav.Strings[1]),0,SND_ASYNC);
          end
          else  //---------------------------------------------------------- 2 приближение
          if SingleBeep  then
          begin
            SingleBeep := false; PlaySound(PAnsiChar(ObjectWav.Strings[0]),0,SND_ASYNC);
          end
          else
          if SingleBeep2 then
          begin
            SingleBeep2 := false; PlaySound(PAnsiChar(ObjectWav.Strings[1]),0,SND_ASYNC);
          end
          else
          if SingleBeep4 then
          begin
            SingleBeep4 := false; PlaySound(PAnsiChar(ObjectWav.Strings[3]),0,SND_ASYNC);
          end
          else
          if SingleBeep5 then
          begin
            SingleBeep5 := false; PlaySound(PAnsiChar(ObjectWav.Strings[4]),0,SND_ASYNC);
          end
          else
          if SingleBeep6
          then
          begin
            SingleBeep6 := false; PlaySound(PAnsiChar(ObjectWav.Strings[5]),0,SND_ASYNC);
          end;
        end;
    ...
    reportf('Ошибка [TabloFormDSP.MainTimerTimer]'); Application.Terminate;
  end;
end;
//========================================================================================
var
...
begin
  try
    ...
    reportf('Ошибка [TabloFormDSP.TTabloMain.RefreshTablo]');
...
try
  ...
  reportf('Ошибка [TabloFormDSP.TTabloMain.BeepTimerTimer]'); Application.Terminate;
end;
end;
...
try
  ...
  reportf('Ошибка [TabloFormDSP.IncrementKOK]');
  Application.Terminate;
end;
end;
...
try
  ...
  reportf('Ошибка [TabloFormDSP.TTabloMain.KeyOtvKomState]'); Application.Terminate;
end;
end;
{$ENDIF}
...
{$ENDIF}
  ...
  reportf('Ошибка [TabloFormDSP.PresetObjParams]'); Application.Terminate;
end;
end;
...
try
  ...
  reportf('Ошибка [TabloFormDSP.ChangeDirectState]'); Application.Terminate;
end;
end;
...
begin
  try
    ...
    reportf('Ошибка [TabloFormDSP.ChangeRegion]'); Application.Terminate;
  end;
end;
//========================================================================================
